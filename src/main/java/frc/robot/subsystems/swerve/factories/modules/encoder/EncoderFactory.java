...
public class EncoderFactory {
	...
	public static IAngleEncoder createEncoder(String logPath, ModuleUtil.ModulePosition modulePosition) {
		...
		return switch (Robot.ROBOT_TYPE) {
			case REPLAY -> switch (modulePosition) {
			...
			case REAL, SIMULATION -> SimulationEncoderBuilder.buildEncoder(logPath);
		};
	}

	public static EncoderSignals createSignals(IAngleEncoder angleEncoder) {
		return switch (Robot.ROBOT_TYPE) {
			case REPLAY -> CANCoderEncoderBuilder.buildSignals((CANCoderEncoder) angleEncoder);
